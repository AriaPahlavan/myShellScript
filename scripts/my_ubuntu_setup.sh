...
if [[ $wish == Y* ]]; then
  ...
  git config --global push.default simple && printf "${grn}[PASS]${nrml} Default git credentials were configured\n" || printf "${red}[FAIL]${nrml} Could not set default git credentials\n"
fi
...
if [[ $wish == Y* ]]; then
  ...
  touch /home/aria/touchpad.sh && chmod +x /home/aria/touchpad.sh && echo "synclient HorizTwoFingerScroll=0 VertTwoFingerScroll=1 HorizEdgeScroll=1 VertEdgeScroll=0 TapButton1=1 TapButton2=0 TapButton3=0" >> /home/aria/touchpad.sh && gsettings set org.gnome.settings-daemon.peripherals.input-devices hotplug-command "/home/aria/touchpad.sh" && printf "${grn}[PASS]${nrml} Done setting up synclient configurations\n" || printf "${red}[FAIL]${nrml} Could not set up synclient configurations\n"
fi
